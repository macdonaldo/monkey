...
impl Parser {
    ...
    fn parse_let_statement(&mut self) -> Result<Statement, String> {
        ...
        Ok(Statement::Let{token, identifier, value})
    }
    ...
    fn parse_return_statement(&mut self) -> Result<Statement, String> {
        ...
        Ok(Statement::Return{token, return_value})
    }
    ...
    fn parse_prefix(&mut self) -> Result<Expression, String> {
        match &self.curr_token {
            ...
            Token::TRUE |
            Token::FALSE => self.parse_boolean_expression(),
            Token::IF => self.parse_if_expression(),
            Token::FUNCTION => self.parse_function_literal_expression(),
            _ => Err(format!("no parse prefix function found for {}", self.curr_token)),
        }
    }
    ...
    fn parse_function_parameters(&mut self) -> Result<Vec<Expression>, String> {
        ...
        if !self.expect_peek(&right_paren) {
            return Err(format!("Expected next token to be {}, got {} instead", right_paren, self.peek_token));
        }
    ...
    fn parse_boolean_expression(&self) -> Result<Expression, String> {
        Ok(Expression::Boolean{token: self.curr_token.clone(), value: self.curr_token_is(Token::TRUE)})
    }
    ...
    fn parse_integer_literal_expression(&self, num_str: &str) -> Result<Expression, String> {
        match i64::from_str_radix(num_str, 10) {
            Ok(value) => Ok(Expression::IntegerLiteral{token: self.curr_token.clone(), value}),
    ...
    fn parse_infix(&mut self, expression: Box<Expression>) -> Result<Expression, String> {
        match &self.curr_token {
            ...
            _ => Err(format!("no parse infix function found for {}", self.curr_token)),
        }
    }
    ...
    fn parse_if_expression(&mut self) -> Result<Expression, String> {
        ...
        Ok(Expression::If{token, condition, consequence, alternative})
    }
