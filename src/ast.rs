...
pub enum Statement {
    Let{token: Token, identifier: Token, value: Expression},
    Return{token: Token, return_value: Expression},
...
impl fmt::Display for Statement {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            Statement::Expression{token:_, expression} => write!(f, "{}", expression),
            Statement::Let{token, identifier, value} => write!(f, "{} {} = {};", token, identifier, value),
            Statement::Return{token, return_value}  => write!(f, "{} {};", token, return_value),
            Statement::Block{token:_, statements} => {
                let s = statements.iter().
...
pub enum Expression {
    ...
    Infix{operator: Token, left: Box<Expression>, right: Box<Expression>},
